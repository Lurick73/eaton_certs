This script allows for easier automatic certificate renewal of the Eaton M3 Network Card using a Python script and Selenium. <br>Make sure to replace the following values as needed within the script:
```
...
```
If True, run each command via a shell (needed for pipes, &&, redirects, etc).<br>If False, commands must be given as arg lists, e.g. ["ls", "-la"].
```
USE_SHELL = True
```
Set True if the commands need root privileges and this script itself is not already running as root (will prefix with `sudo`). <br> Requires either passwordless sudo for this user, or that you run this whole script with sudo/as root to begin with.
```
RUN_WITH_SUDO = False
...
Parts of the script were created with Claude.ai
